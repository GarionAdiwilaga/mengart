# Mengart Blueprint 2.2.1 — Re-Baselined Production Remediation Plan

**Product baseline:** Art Community App Implementation Blueprint 2.2.1  
**Approved historical Gate A commit:** `15459ecfdb2e4bf2f22b16464b383ddf55e08c1c`  
**Overall production status:** **NO-GO** until Gates B–H and final independent audit pass.  
...
Blueprint 2.2.1 changes **target product behavior**, not historical facts. Therefore:
...
3. **Reframe findings whose old acceptance criterion encoded Blueprint 2.1 behavior.** The QA risk remains useful, but the fix must follow Blueprint 2.2.1.
...
Blueprint 2.2.1 removes official #2/#3/lower Community placements.
...
The following matrix preserves the Independent Final QA IDs but updates their meaning against Blueprint 2.2.1.

| QA ID | New status | Revised gate | Blueprint 2.2.1 interpretation |
|---|---|---|---|
...
The old QA report remains a **risk and evidence catalog**, not a frozen product specification. When an old QA acceptance criterion conflicts with Blueprint 2.2.1, preserve the underlying safety/integrity concern and rewrite the acceptance criterion around the new product flow.
...
- Phase A regression suite remains green except assertions explicitly superseded by v2.2.1.
...
- Google OAuth only.
- First Google login enters `PENDING_INVITE` gate.
- Invite code can be entered manually.
- Invite URL stores code across Google redirect then redeems automatically after login.
- Discord-style invite fields: secure token/code, label, optional expiry, optional max uses, used count, revocation, redemption history.
- Long-term membership states: `ACTIVE`, `SUSPENDED`, `DELETED`.
- Remove active password/SMTP/email-verification/password-reset flows after safe data review.

#### Roles

**Admin:** system administrator, full platform/system access.  
**Moderator:** community operations only; no system/deployment/security configuration authority.  
...
Blueprint 2.2.1 is the product authority.
...
1. Gates B–H all pass independent QA;
2. all retained P0/P1 safety/integrity risks are closed under their revised acceptance criteria;
3. the complete application passes a final independent audit against Blueprint 2.2.1;
