...
- **Phase 2: Release Gate B (Voting & Tie Resolution — Blueprint 2.2.1):** **TARGETED CORRECTIONS COMPLETED & 100% VERIFIED**
  ...
  - Production Domain Services (`src/lib/services/votingService.ts`):
    ...
    - `resetBallotService`: Resets ballot allocations for the specific round with active membership check.
  ...
  - Lifecycle & Scheduler Updates (`src/lib/services/challengeService.ts`):
    - Inert PAUSED behavior: `'paused'` removed from active operational transitions and rejected.
    - Scheduler-authoritative voting opening: generic status transition blocked from manually entering `'voting_open'`.
    - System actor context `{ userId: null, role: "system" }` writes valid `actor_id = NULL` to `audit_logs`.
  ...
  - UI & Components:
    - `VotingWorkspace.tsx` and server actions require clean `{ votingRoundId, votes }` and `{ votingRoundId }` signatures.
