...
export function RevokeInviteButton({ inviteId, code }: RevokeInviteButtonProps) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="w-full max-w-md glass-panel-elevated p-6 rounded-3xl flex flex-col gap-5">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              Tindakan ini akan membatalkan status aktif undangan secara permanen. Calon anggota yang belum menukarkan undangan ini tidak akan dapat menggunakannya lagi.
            </p>
            ...
            <form onSubmit={handleRevoke} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-mono text-zinc-400">
                  Alasan Pencabutan <span className="text-red-400">*</span>
                </label>
                <textarea
                  value={reason}
                  onChange={(e) => setReason(e.target.value)}
                  placeholder="Misal: Kuota telah terisi via channel lain atau salah peruntukan."
                  rows={3}
                  className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white text-xs placeholder:text-zinc-600 focus:outline-none focus:border-amber-500/50 resize-none font-sans"
                  required
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="px-4 py-2 rounded-xl bg-red-500 hover:bg-red-400 text-white text-xs font-bold transition-all shadow-md shadow-red-500/20 flex items-center gap-2 cursor-pointer disabled:opacity-50"
                >
