...
export const challengeResults = pgTable(
  "challenge_results",
  {
    ...
    awardType: text("award_type").default("community_vote_winner").notNull(), // 'community_vote_winner' | 'jury_award'
  ...
  (table) => [
    index("idx_challenge_results_challenge_id").on(table.challengeId),
    index("idx_challenge_results_submission_id").on(table.submissionId),
    index("idx_challenge_results_award_type").on(table.awardType),
    index("idx_challenge_results_jury_award_id").on(table.juryAwardId),
