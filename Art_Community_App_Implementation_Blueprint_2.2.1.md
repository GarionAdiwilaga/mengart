...
**Supersedes:** Blueprint 2.2.0 and Blueprint 2.1.0 for product rules described here  
...
**2.2.1 revision:** Community voting has exactly one official result concept: the Community Vote Winner. Lower-place public ranks/highlights (#2/#3/etc.) are removed. Raw Star totals remain available for live display and audit, but manual/tiebreak resolution never manufactures lower-rank ordering.
...
not authenticated
    ↓
store pending invite securely for OAuth continuation
    ↓
Google Login
    ↓
OAuth callback
...
Implementation may use a short-lived signed/HttpOnly cookie or equivalent server-side OAuth continuation state. The raw invite token must not be written to logs.
...
Invitations intentionally resemble Discord server invites but remain simple.

Each invitation contains:

- Random high-entropy token/code.
- Stored token hash; raw token need not be retained.
- Optional label.
- Optional expiry.
- Optional maximum-use count; may be unlimited.
- Used count/redemption history.
- Creator.
- Creation timestamp.
- Optional revocation timestamp/actor.

Rules:

- Revalidate invitation at redemption time.
- Enforce usage limits transactionally.
- Do not create partial ACTIVE memberships after failed redemption.
- An already ACTIVE user does not consume another invite merely by opening an invite link.
- If a raw invite link is lost, staff may create a replacement instead of requiring raw-token recovery.
...
- [ ] Invite-link continuation survives Google OAuth without exposing raw token in logs.
...
Keep the approved migration history, transactional foundations, scheduler/cron safety, audit foundations, voting-round identity, and frozen-candidate model. Product behavior later superseded by Blueprint 2.2.1 is adapted forward rather than retroactively rewriting Gate A migrations.
...
- Discord-style reusable invite code/link flow.
...
After Gate H, perform a final independent audit against **Blueprint 2.2.1**, not superseded Blueprint 2.1/2.2.0 rules.
...
1. Treat **v2.2.1 as the authoritative product rule baseline**.
...
7. Add automated tests for every lifecycle, voting, jury, invitation, and permission rule changed by v2.2.
